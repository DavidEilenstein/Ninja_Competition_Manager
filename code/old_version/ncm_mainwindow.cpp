...
void NCM_MainWindow::on_pushButton_SIDE_ResultsEntering_clicked()
{
    if(!competition->is_valid())
        return;

    pResultEntering_sidechallenge = new NCM_ResultEntering_SideChallange(competition->dir_competition());
    this->close();
    pResultEntering_sidechallenge->show();
}

void NCM_MainWindow::on_pushButton_SIDE_Ranking_clicked()
{

}

void NCM_MainWindow::on_pushButton_SIDE_edit_clicked()
{
    if(!competition->is_valid())
        return;

    NCM_SideChallenge SideChallenge(competition->dir_competition(), competition->competitor_classes());
    this->close();
    SideChallenge.exec();
}
